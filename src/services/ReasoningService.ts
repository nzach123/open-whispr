...
class ReasoningService extends BaseReasoningService {
  ...
  private async processWithGemini(
    ...
    try {
      // Use agent's system prompt if provided, otherwise default
      const systemPrompt = config.agentSystemPrompt
        ? config.agentSystemPrompt.replace(/\{\{text\}\}/g, text).replace(/\{\{agentName\}\}/g, agentName || '')
      ...
      // For agent mode, use the system prompt directly; for regular mode, use getReasoningPrompt
      const userContent = config.agentSystemPrompt
        ? text  // Agent's system prompt already contains everything
        : this.getReasoningPrompt(text, agentName, config);

      const requestBody = {
        contents: [{
          parts: [{
            text: config.agentSystemPrompt ? systemPrompt : `${systemPrompt}\n\n${userContent}`
          }]
        }],
        generationConfig: {
